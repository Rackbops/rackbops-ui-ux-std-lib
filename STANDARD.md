...
| `form.css` | `.rb-field`, `.rb-label`, `.rb-input`, `.rb-textarea`, `.rb-select`, `.rb-choice`, `.rb-checkbox`, `.rb-radio`, `.rb-switch` | `Field`, `Label`, `Input`, `Textarea`, `Select`, `Checkbox`, `Radio`, `Switch` | Checkbox and radio SHOULD use `accent-color` (nine themes do; the concrete pair draws checked states as a flat accent fill by design, `styles/concrete-signal/design.md:123-124`; the ports hand-style them). The switch is a track-and-thumb control in every theme, never `accent-color` on a widened native checkbox: the shared shape for the nine `accent-color` themes, the concrete pair's own square drawing, and each port's own upstream drawing (#209) |
