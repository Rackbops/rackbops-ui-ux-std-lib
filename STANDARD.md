...
| `form.css` | `.rb-field`, `.rb-label`, `.rb-input`, `.rb-textarea`, `.rb-select`, `.rb-choice`, `.rb-checkbox`, `.rb-radio`, `.rb-switch`, `.rb-field__help`, `.rb-field__error`, `.rb-label__required` | `Field`, `Label`, `Input`, `Textarea`, `Select`, `Checkbox`, `Radio`, `Switch`, `FieldHelp`, `FieldError` | Checkbox and radio SHOULD use `accent-color` (nine themes do; the concrete pair draws checked states as a flat accent fill by design, `styles/concrete-signal/design.md:123-124`; the ports hand-style them). The switch is a track-and-thumb control in every theme, never `accent-color` on a widened native checkbox: the shared shape for the nine `accent-color` themes, the concrete pair's own square drawing, and each port's own upstream drawing (#209). The invalid state is the `aria-invalid="true"` attribute on `.rb-input`/`.rb-select`/`.rb-textarea` in every theme, never a class (#210, §5.5); error text is ink (`--rb-text`) with a `--rb-danger` left bar, never `--rb-danger`-coloured text (§9); the required marker is an `aria-hidden` glyph rendered by `Label`'s own `required` prop, with the control's own `required` attribute carrying the programmatic state for assistive tech |
...
| `toast.css` | `.rb-toast`, `--info`, `--success`, `--warning`, `--danger`, `__close`; `.rb-toast-region` | `Toast`, `ToastRegion` | A floating transient notice stacked in the fixed `.rb-toast-region` (which is not itself a live region). Role by semantic, set in React: info/success are `role="status"` (polite), warning/danger `role="alert"` (assertive). Never colour-only -- the message text carries the meaning, and it is never the only place an error is reported. The card echoes each theme's `alert.css`, elevated; give-back #211 |
...
- Documented under `### Theme extras` in that theme's `design.md` (nine of
  ...
  accurate]`.
- Never emitted by a shared React component. `LinksIndex` and `EmptyState`
  are the models: each composes `Card` (and `Badge`, for `LinksIndex`) and
  adds no class of its own (`components/react/src/LinksIndex.tsx:67` (`Card`)
...
- render every required selector in section 5.1 in every state the CSS
  ...
  (#50); and the Toasts section renders all four semantic variants, each
  dismissible, in a static demo of the fixed `.rb-toast-region` (#211);
...
| React prop -> class mapping, composition, passthrough | `src/*.test.tsx` | live (Button, NavRail, LinksIndex, Stepper, EmptyState, Toast) |
