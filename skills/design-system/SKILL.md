...
| `Field`/`Label`/`Input`/`Textarea`/`Select`/`Checkbox`/`Radio`/`Switch`/`FieldHelp`/`FieldError` | `.rb-field` | pair with Field/Label; choice controls wrap in .rb-choice; className/rest still target the control itself -- use wrapperClassName/wrapperStyle to style the .rb-choice row (#84). Invalid is the aria-invalid="true" attribute on the control (Input/Select/Textarea), styled in every theme, never a class (#210). FieldHelp/FieldError are plain styled <p>s with no live-region role -- the consumer wires aria-describedby/aria-invalid on the control. rb-label__required is rendered by Label's own required prop, an aria-hidden glyph; the control's own required attribute carries the programmatic state for assistive tech. |
...
| `Toast`/`ToastRegion` | `.rb-toast` | floating transient notice, a give-back from the consoles that each built their own (#211); role chosen by semantic (info/success = status/polite, warning/danger = alert/assertive); ToastRegion is the fixed stack and is deliberately not itself a live region; optional __close dismiss button via onDismiss |
