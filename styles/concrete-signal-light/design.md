...
`.rb-switch`/`.rb-field__help`/`.rb-field__error`/`.rb-label__required` — invalid
is `[aria-invalid="true"]` with the theme's own kept 1px border rule, same as
concrete-signal, see its `design.md` for the full #210 note), `.rb-badge`, `.rb-alert`, `.rb-toast`, `.rb-dialog`, `.rb-tabs`/`.rb-tab`/
