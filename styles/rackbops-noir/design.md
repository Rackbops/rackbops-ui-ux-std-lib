...
The token pairs `styles/test/contrast.test.mjs` computes clear their WCAG targets, with the below-AA faint pairs the test emits as warnings (not failures):
...
- **`--rb-text-faint` on `--rb-surface-2` is ~3.6:1** (3.57) -- below AA, above the 3:1 floor; same rule.
...
`.rb-alert`, `.rb-toast`, `.rb-dialog`, `.rb-tabs`, `.rb-tabstrip` (+ `__tab`, `--active`, `__badge`), `.rb-wordmark` (+ `__spark`; solid `--rb-text` at `1rem`, system sans weight 600 — the gradient stays on the rack bars), `.rb-table` (+ `.rb-num`, `--interactive`, `__group-row`, `__sort`, `.rb-table-scroll`), `.rb-progress`/`.rb-spinner`,
